...
export default function NewJobPage() {
  ...
  const [strategy, setStrategy] = useState('equivalence');
  ...
  return (
    <div className="w-full max-w-5xl mx-auto px-4 sm:px-6 py-8 space-y-8 bg-paper">
      ...
      {/* Pipeline Timeline — shown once launched */}
      {isLaunching && (
        <JobProgressTimeline
          running={isLaunching && !pipelineDone}
      ...
      {/* Success message */}
      {pipelineDone && (
        <div className="border-3 border-green-500 bg-green-50 p-5 font-mono text-xs text-green-700 font-bold animate-fade-slide-in flex items-center gap-3">
          <span className="text-2xl">✓</span>
          <span>Pipeline launched successfully! Redirecting to Jobs workspace…</span>
        </div>
      )}
        ...
        <form onSubmit={handleLaunch} className="space-y-8">
          ...
          <div className="border-3 border-ink bg-white p-6 shadow-[6px_6px_0_0_#0A0A0A] space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4 max-w-md">
              <button
                type="button"
                onClick={() => { setProvider('github'); setSelectedRepo(MOCK_GITHUB_REPOS[0]); }}
                className={`p-4 border-3 border-ink font-mono text-xs font-bold uppercase flex items-center justify-center gap-3 transition-all ${
                  provider === 'github' ? 'bg-ink text-paper shadow-[4px_4px_0_0_#FF2D00]' : 'bg-white text-ink hover:bg-paper/50'
                }`}
              >
              ...
              <button
                type="button"
                onClick={() => { setProvider('gitlab'); setSelectedRepo(MOCK_GITLAB_REPOS[0]); }}
                className={`p-4 border-3 border-ink font-mono text-xs font-bold uppercase flex items-center justify-center gap-3 transition-all ${
                  provider === 'gitlab' ? 'bg-ink text-paper shadow-[4px_4px_0_0_#FF2D00]' : 'bg-white text-ink hover:bg-paper/50'
                }`}
              >
          ...
          <div className="border-3 border-ink bg-white p-6 shadow-[6px_6px_0_0_#0A0A0A] flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <button
              type="submit"
              className="w-full sm:w-auto px-8 py-4 border-3 border-ink bg-ink text-paper font-display font-bold text-xs uppercase tracking-wider hover:bg-signal hover:border-signal hover:text-white transition-all shadow-[5px_5px_0_0_#0A0A0A] cursor-pointer"
            >
